...
In addition to modeling the existing pilot program, we created other scenarios to analyze. A map showing the zones we analyzed is given in Figure \@ref(fig:zone-map). For each of these zones we defined a microtransit fleet, including fleet size and operating hours, which can be seen in Table \@ref(tab:zone-fleets). We were less concerned with modeling each of these zones individually, but rather how the addition of another zone would affect the whole. We therefore created several scenarios as combinations of these areas, which are given in Table \@ref(tab:scenarios-info).
