...
ggplot(aes(x = variable, y = statistic, color = scenario, pch = result)) + 
  ...
  ylab("Share (ish)") + 
  scale_color_manual(name = "Scenario", values = wes_palette("Zissou1", 5, type = "continuous")) + 
...
fl_share <- left_join(
  ...
  mutate(
    transfer = ifelse(is.na(transfer), 0, transfer),
    share = transfer / total 
  )
...
fl_share %>%
  ...
  filter(!grepl("D", scenario)) %>%
  mutate(scenario = gsub(" 5 it", "", scenario)) %>%
ggplot(aes(x = area, y = share)) +
  geom_col() +
  facet_wrap(~scenario, scales = "free_x") + 
  scale_y_continuous(labels = scales::percent) + 
  xlab("Area") + ylab("Percent with transit connection")
